...
class PathEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        print("[DEBUG] PathEncoder in_channels =", in_channels)  # 加这一行看看
        # 1) GRU 编码节点特征
        self.lin = nn.GRU(
...
class PathMILModel(nn.Module):
    """
    - encoder: PathEncoder（图 -> 路径 embedding）
    - path_cls: 把每条路径 embedding 映射到一个 logit（表示该路径含漏洞的“软标签”）
    """
    def __init__(self, args):
        super().__init__()
        self.encoder = PathEncoder(args)
    ...
    def forward(self, batch):
        path_emb = self.encoder(batch.x, batch.edge_index, batch.batch)
        logits = self.classifier(path_emb).squeeze(-1)
        return logits

    # def __init__(self, args):
    #     super().__init__()
    #     self.encoder = PathEncoder(args)
    #     self.path_cls = nn.Linear(args.node_hidden_dim, 1)

    # def forward(self, data):
    #     """
    #     data 需要至少包含：
    #     - data.x           [N_nodes, feat_dim]
    #     - data.edge_index  [2, N_edges]
    #     - data.batch       [N_nodes]    # PyG 的 batch：节点 -> 路径图 id
    #     """
    #     x          = data.x
    #     edge_index = data.edge_index
    #     batch      = data.batch

    #     path_emb   = self.encoder(x, edge_index, batch)   # [num_paths, hidden]
    #     path_logits = self.path_cls(path_emb).squeeze(-1) # [num_paths]
    #     return path_logits
